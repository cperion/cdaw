...
diag.status("view.grid_patch_view.to_decl", "real")
local V = D.View
...
local function lower(self, ctx)
    return diag.wrap(ctx, "view.grid_patch_view.to_decl", "real", function()
        ...
        -- Add module button
        local add_cmd = C.find_command(self.commands, V.GPCCAddModule)
        ...
        return P.fallback_node(ctx, C.identity_key(self.identity), "view.grid_patch_view.to_decl", tostring(err))
    end)
end
