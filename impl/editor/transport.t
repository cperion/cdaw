...
local lower_transport = terralib.memoize(function(self)
...
local lower_tempo_map = terralib.memoize(function(self)
    local tempo = L()
...
function D.Editor.Transport:lower()
    return diag.wrap(nil, "editor.transport.lower", "real", function()
        return lower_transport(self)
    end, function()
        return F.authored_transport()
    end)
end

function D.Editor.TempoMap:lower()
    return diag.wrap(nil, "editor.tempo_map.lower", "real", function()
        return lower_tempo_map(self)
    end, function()
