...
local L = F.L
diag.status("authored.mod_slot.resolve", "stub")


function D.Authored.ModSlot:resolve(ctx)
    return diag.wrap(ctx, "authored.mod_slot.resolve", "stub", function()
        ...
        for i = 1, #self.routings do
            ...
            routes[i] = D.Resolved.ModRoute(
                0,                 -- mod_slot_index (set by parent)
                r.target_param_id,
                r.depth,
                r.bipolar,
                r.scale_mod_slot,
        ...
        return D.Resolved.ModSlot(
            slot_index,
            0,                    -- parent_node_id (set by parent)
            mod_node.id,
            self.per_voice,
