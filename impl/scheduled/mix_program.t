-- impl/scheduled/mix_program.t
-- Scheduled.MixProgram:compile -> Kernel.Unit
...
diag.status("scheduled.mix_program.compile", "real")
...
local compile_mix_program = terralib.memoize(function(self)
    ...
    local body_q = compile_mix_job(self.mix, ctx)
...
function D.Scheduled.MixProgram:compile()
    return diag.wrap(nil, "scheduled.mix_program.compile", "real", function()
        return compile_mix_program(self)
    end, function()
