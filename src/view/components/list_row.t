...
function M.button_row(scope, text, action, opts)
    opts = opts or {}
    local p = C.palette()
    return C.ui.button {
        key = scope,
        width = opts.width or C.ui.grow(),
        height = opts.height or C.ui.fixed(22),
        padding = opts.padding or { left = 8, top = 0, right = 8, bottom = 0 },
        text = text,
        action = action,
        background = opts.background or p.surface_panel,
        border = opts.border or C.ui.border { bottom = 1, color = p.border_subtle },
        text_color = opts.text_color or p.text_primary,
        font_size = opts.font_size or 11,
    }
end

function M.value_row(scope, lhs_text, rhs_node, opts)
    opts = opts or {}
    local p = C.palette()
    return C.ui.row {
        key = scope,
...
function M.action_value(scope, lhs_text, action, opts)
    opts = opts or {}
    return M.value_row(scope, lhs_text,
        B.flat_button(opts.button_text or "Edit", action, {
            key = scope:child("action"),
            width = opts.button_width or C.ui.fixed(40),
            height = C.ui.fixed(18),
