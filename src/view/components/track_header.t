-- impl/view/components/track_header.t
-- Shared track-header lowering atom used by multiple View surfaces.


local C = require("src/view/common")
local T = require("src/view/components/text")
local B = require("src/view/components/button")
local I = require("src/view/components/icons")
local P = require("src/view/components/placeholder_panel")

local M = {}

function M.lower(track_header, selection)
    local ui = C.ui
    local p = C.palette()
    local scope = C.make_scope(track_header.identity, "track_header")
    local selected = C.selection_is_track(selection, track_header.track_ref)
    local select_cmd = C.find_command(track_header.commands, "THCCSelectTrack")

    local role_label = nil
    local height = 30
    local button_w = 16
    if track_header.role.kind == "ArrangementHeaderRole" then
        role_label = "HYBRID TRACK"
        height = 36
    elseif track_header.role.kind == "LauncherHeaderRole" then
        role_label = nil
        height = 28
    elseif track_header.role.kind == "MixerHeaderRole" then
        role_label = nil
        height = 26
    end

    local children = {
        ui.column {
            key = scope:child("color"),
            width = ui.fixed(3),
            height = ui.grow(),
            background = p.track_accent,
        } {},
        ui.column {
            key = scope:child("body"),
            width = ui.grow(),
            height = ui.grow(),
            gap = role_label and 1 or 0,
        } {
            T.strong_label(C.track_name(track_header.track_ref), {
                key = scope:child("title"),
                width = ui.grow(),
                font_size = 11,
            }),
            role_label and T.quiet_label(role_label, {
                key = scope:child("meta"),
                width = ui.grow(),
                font_size = 9,
                text_color = p.text_muted,
            }) or nil,
        },
        I.icon_button(scope:child("solo"), I.solo, nil, {
            size = button_w, icon_size = 14,
            icon_color = p.text_secondary,
        }),
        I.icon_button(scope:child("mute"), I.mute,
            select_cmd and select_cmd.action_id or nil, {
            size = button_w, icon_size = 14,
            icon_color = selected and p.text_primary or p.text_secondary,
        }),
    }
    P.overlay_children(scope, track_header.identity, children)

    return ui.row {
        key = scope,
        width = ui.grow(),
        height = ui.fixed(height),
        padding = { left = 6, top = 3, right = 6, bottom = 3 },
        gap = 6,
        align_y = ui.align_y.center,
        background = selected and p.surface_selected or p.surface_track_header,
        border = C.border( selected and p.border_selected or p.border_track_header, 1),
    } (children)
end

return M
