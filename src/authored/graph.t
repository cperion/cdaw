...
return function(R)
    ...
    local function push(tbl, item) local idx = #tbl; tbl[idx + 1] = item; return idx end
    local function to_list(tbl) local l = L(); for i = 1, #tbl do l:insert(tbl[i]) end; return l end

    local function make_state()
        return {
            next_curve_id = 0, next_mod_slot_index = 0,
            graphs = {}, graph_ports = {}, nodes = {}, child_graph_refs = {},
            wires = {}, params = {}, mod_slots = {}, mod_routes = {}, curves = {},
        }
    end
    ...
    local function resolve_param_internal(param, ticks_per_beat, node_id, st)
        ...
        local curve_id = nil
        if curve then
            curve_id = st.next_curve_id; st.next_curve_id = curve_id + 1
            push(st.curves, R.AutoCurve(curve_id, curve.points, curve.interp_code))
        end
        return R.Param(rp.id, node_id, rp.name, rp.default_value, rp.min_value, rp.max_value,
            R.ParamSourceRef(rp.source.source_kind, rp.source.value, curve_id),
            rp.combine_code, rp.smoothing_code, rp.smoothing_ms)
    end

    local function encode_mod_args(kind)
        if not kind or not kind.kind then return 0, 0, 0, 0 end
        if kind.kind == "LFOMod" then
            local shape_name = kind.shape and kind.shape.kind or "Sine"
    ...
    local function resolve_mod_slot_internal(ms, ticks_per_beat, parent_node_id, st)
        local slot_index = st.next_mod_slot_index; st.next_mod_slot_index = slot_index + 1
        ...
        for i = 1, #ms.routings do
            ...
            push(st.mod_routes, R.ModRoute(slot_index, r.target_param_id, r.depth, r.bipolar, r.scale_mod_slot, r.scale_param_id))
        end
        local kind_ref = ms.modulator.kind:resolve()
        local arg0, arg1, arg2, arg3 = encode_mod_args(ms.modulator.kind)
        return R.ModSlot(slot_index, parent_node_id, ms.modulator.id, kind_ref.kind_code,
            first_param, #ms.modulator.params, arg0, arg1, arg2, arg3,
            ms.per_voice, first_route, #ms.routings)
    end
    ...
    local function resolve_node_internal(node, ticks_per_beat, st)
        ...
        for i = 1, #node.child_graphs do
            ...
            push(st.child_graph_refs, R.ChildGraphRef(child_root.id, role_codes[cg.role and cg.role.kind] or 0))
        end
        return R.Node(node.id, kind_ref.kind_code, first_param, #node.params,
            0, #node.inputs, 0, #node.outputs,
            first_mod_slot, #node.mod_slots,
            first_child_graph_ref, #node.child_graphs,
            node.enabled, nil,
            node.x_pos or 0, node.y_pos or 0, 0, 0)
    end
    ...
    resolve_graph_internal = function(graph, ticks_per_beat, st)
        ...
        for i = 1, #graph.inputs do
            ...
            push(st.graph_ports, R.GraphPort(p.id, p.name, hint_codes[p.hint and p.hint.kind] or 0, p.channels, p.optional))
        end
        ...
        for i = 1, #graph.nodes do
            ...
            push(st.nodes, rn); node_ids:insert(rn.id)
        end
        local wire_ids = L()
        for i = 1, #graph.wires do
            local w = graph.wires[i]
            local wire_idx = push(st.wires, R.Wire(w.from_node_id, w.to_node_id))
            wire_ids:insert(wire_idx)
        end
        local rg = R.Graph(graph.id,
            layout_codes[graph.layout.kind or graph.layout] or 0,
            domain_codes[graph.domain.kind or graph.domain] or 1,
            port_base, #graph.inputs, port_base + #graph.inputs, #graph.outputs,
            node_ids, wire_ids, 0, #graph.pre_cords, 0, 0, 0, 0)
        push(st.graphs, rg)
        return rg
    end
    ...
    return function(self, ticks_per_beat)
        ...
        return R.GraphSlice(to_list(st.graphs), to_list(st.graph_ports), to_list(st.nodes),
            to_list(st.child_graph_refs), to_list(st.wires), to_list(st.params),
            to_list(st.mod_slots), to_list(st.mod_routes), to_list(st.curves))
    end
end
